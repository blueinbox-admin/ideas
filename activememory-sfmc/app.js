// app.js — interactive logic for the Vanta Motors Marketing Memory demo (SFMC, read-only).
...
//                  refreshProjects, flashProject  (the SFMC project rail)
...
(function (AM) {
  ...
  function updateBadges() {
    byId('bReview').textContent = reviewPanel.querySelectorAll('.card.qcard:not(.leaving):not(.scope-hidden), .card.suggested:not(.leaving):not(.scope-hidden)').length;
    byId('bFacts').textContent  = factsPanel.querySelectorAll('.card:not(.leaving):not(.scope-hidden)').length;
    byId('bGuard').textContent  = guardPanel.querySelectorAll('.card:not(.leaving):not(.scope-hidden)').length;
  }

  // ---------- projects rail (the SFMC scope filter) ----------
  // Every card carries data-scope (a project key or 'org'). The rail filters all
  // three tabs to one project; org-wide facts are INHERITED, so they also show
  // under any project. Chip counts show what's filed exactly under each scope.
  var projbar = byId('projbar');
  var scopePanels = [reviewPanel, factsPanel, guardPanel];
  var currentProj = 'all';
  function allCards() {
    var out = [];
    scopePanels.forEach(function (p) { if (p) p.querySelectorAll('.card').forEach(function (c) { out.push(c); }); });
    return out;
  }
  function applyProjFilter() {
    allCards().forEach(function (c) {
      var s = c.getAttribute('data-scope') || 'org';
      var show = currentProj === 'all' || s === currentProj || (currentProj !== 'org' && s === 'org');
      c.classList.toggle('scope-hidden', !show);
    });
  }
  ...
  function flashProject(scope) {
    if (!projbar) return;
    var chip = projbar.querySelector('.proj-chip[data-proj="' + scope + '"]');
    if (chip) { chip.classList.remove('flash'); void chip.offsetWidth; chip.classList.add('flash'); setTimeout(function () { chip.classList.remove('flash'); }, 3300); }
  }
  if (projbar) projbar.addEventListener('click', function (e) {
    var chip = e.target.closest('.proj-chip'); if (!chip) return;
    stopStory();
    projbar.querySelectorAll('.proj-chip').forEach(function (c) { c.classList.toggle('active', c === chip); });
    currentProj = chip.getAttribute('data-proj');
    refreshProjects();
  });
  refreshProjects();
  ...
  // ---------- card helpers ----------
  function makeCard(text, type, scope) {
    scope = scope || 'org';
    var card = document.createElement('div'); card.className = 'card fresh'; card.setAttribute('data-scope', scope);
    ...
    // "Applies to" chip — a hand-added fact is Org-wide by default (matches memoryapp,
    // where a fact added at org-root carries the org scope).
    var srow = document.createElement('div'); srow.className = 'scope-row';
    var slab = document.createElement('span'); slab.className = 'scope-row-label'; slab.textContent = 'Applies to';
    var schip = document.createElement('span'); schip.className = 'scope-chip' + (scope === 'org' ? ' org' : '');
    schip.textContent = scope === 'org' ? 'Org-wide' : scope;
    srow.appendChild(slab); srow.appendChild(schip); body.appendChild(srow);
  ...
  document.querySelector('.desktop').addEventListener('click', function (e) {
    ...
    if (card) {
      ...
      if (label === 'Confirm') { var txt = card.querySelector('.content').textContent; var sc = card.getAttribute('data-scope') || 'org'; collapse(card, 'Confirmed'); factsPanel.insertBefore(makeCard(txt, 'fact', sc), factsNote); updateBadges(); refreshProjects(); flashTab('facts'); }
  ...
  AM.convo = convo;
  AM.refreshProjects = refreshProjects;
  AM.flashProject = flashProject;
})(window.AM);
